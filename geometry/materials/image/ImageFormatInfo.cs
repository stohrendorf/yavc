...
public sealed class ImageFormatInfo
{
    ...
    )
    {
        ...
        _is8Aligned = redBitsPerPixel is 0 or 8 &&
                      greenBitsPerPixel is 0 or 8 &&
                      blueBitsPerPixel is 0 or 8 &&
                      alphaBitsPerPixel is 0 or 8;

        _is16Aligned = redBitsPerPixel is 0 or 16 &&
                       greenBitsPerPixel is 0 or 16 &&
                       blueBitsPerPixel is 0 or 16 &&
                       alphaBitsPerPixel is 0 or 16;

        _is32Aligned = redBitsPerPixel is 0 or 32 &&
                       greenBitsPerPixel is 0 or 32 &&
                       blueBitsPerPixel is 0 or 32 &&
                       alphaBitsPerPixel is 0 or 32;

        if (_is8Aligned || _is16Aligned || _is32Aligned) return;
        ...
        _masks = [dict['b'], dict['g'], dict['r'], dict['a']];
    }
    ...
    internal static ImageFormatInfo? FromFormat(ImageFormat imageFormat)
    {
    ...
    internal int GetDataSize(int width, int height)
    {
        switch (Format)
        {
    ...
    internal byte[] ConvertToBGRA32(byte[] data, int width, int height)
    {
        ...
        switch (Format)
        {
            ...
            default:
            {
                ...
                if (BitsPerPixel > 32) throw new NotImplementedException($"Unsupported format: {Format}");
